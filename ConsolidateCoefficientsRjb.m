...
if model == 3
    directory = fullfile(cd,'FinalCoefficients\Coefficients_Rjb\3BranchModel\');
elseif model == 5
    directory = fullfile(cd,'FinalCoefficients\Coefficients_Rjb\5BranchModel\');
end
...
for wi = 1:2
    stemcoeff=['coeffukrjbboth_',num2str(model,'%1i'),'branches_',char(weightopt(wi)),'_'];
    for Ti = 1:length(Ts)
        for bri = 1:model
            branch = bri;
            ri = ri + 1;
...
if model == 3
    ...
    save(strcat(cd,'\DATDT24_Coefficients.mat'),"DATDT24_Coefficients3b_Rjb",app)
elseif model == 5
    ...
    save(strcat(cd,'\DATDT24_Coefficients.mat'),'DATDT24_Coefficients5b_Rjb',app)
elseif model == 216
    ...
    save(strcat(cd,'\DATDT24_Coefficients.mat'),'DATDT24_Coefficients216b_Rjb',app)
end
