...
if model == 3
    directory=strcat(cd,'\GAB - Rjb coeff files\');
elseif model == 5
    directory=strcat(cd,'\GAB - Rjb coeff files\');
end
...
for wi = 1:2
    stemcoeff=['coeffukrjbboth_',num2str(model,'%1i'),'branches_',char(weightopt(wi)),'_gab_'];
    for Ti = 1:length(Ts)
        for bri = 1:model
            branch = bri;
            ri = ri + 1;
...
if model == 3
    ...
    save(strcat(cd,'\DATDT24_Coefficients.mat'),"DATDT24_Coefficients3b_Rjb",'-append')
    % save(strcat(cd,'\DATDT24_Coefficients.mat'),"DATDT24_Coefficients3b_Rjb")
elseif model == 5
    ...
    save(strcat(cd,'\DATDT24_Coefficients.mat'),'DATDT24_Coefficients5b_Rjb','-append')
elseif model == 216
    ...
    save(strcat(cd,'\DATDT24_Coefficients.mat'),'DATDT24_Coefficients216b_Rjb','-append')
end
